...
class GetRepaymentPlanUseCase {
    fun execute(loan: AnnuityLoan): RepaymentPlan {
        ...
        val totalAmortizationPayment =
            BigDecimal(min(loan.getAmortization(forMonth = loan.years * 12), loan.amount).toString()).setScale(
                2,
                RoundingMode.HALF_UP
            )
        val remainingDebt =
            BigDecimal(loan.amount.toString()).subtract(totalAmortizationPayment).setScale(2, RoundingMode.HALF_UP)
